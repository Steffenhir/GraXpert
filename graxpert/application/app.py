...
class GraXpert:
    ...
    def on_create_grid_request(self, event=None):
        ...
        self.cmd = Command(
            SEL_POINTS_HANDLER, self.cmd, data=self.images["Original"].img_array, num_pts=self.prefs["bg_pts_option"], tol=self.prefs["bg_tol_option"], sample_size=self.prefs["sample_size"]
        )
        self.cmd.execute()
